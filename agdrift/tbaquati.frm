...
Begin VB.Form frmTBAquatic 
   ...
   Begin VB.Frame fraPond 
      Caption         =   "Aquatic Body Definition"
      Height          =   2295
      Left            =   120
      TabIndex        =   16
      Top             =   120
   ...
   Begin VB.Frame fraCalc 
      ...
      Begin VB.Label lblCalcDistUnits 
         ...
         Width           =   420
      End
      Begin VB.Label lblResults3 
         Alignment       =   2  'Center
         Caption         =   "Initial Average Concentration:"
         Height          =   465
         Left            =   360
         TabIndex        =   20
         Top             =   1800
      ...
      Begin VB.Label lblResults1 
         Alignment       =   2  'Center
         Caption         =   "Distance To Water Body From Edge of Application Area:"
         Height          =   435
         Left            =   120
         TabIndex        =   21
         Top             =   285
   ...
   End
End
Attribute VB_Name = "frmTBAquatic"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
' $Id: tbaquati.frm,v 1.13 2003/08/22 19:49:05 tom Exp $
'
...
'Calculate the Toxicity (exposure)
  ...
  AQTYPE = CLng(BodyType) 'This var is maintained by the form
  INTYPE = CLng(CalcOutputMarker) 'This var is maintained by the form
  LPOND = UnitsInternal(Val(txtDist.Text), UN_LENGTH)
  DPOND = UnitsInternal(Val(txtDepth.Text), UN_LENGTH)
  XACT = UnitsInternal(Val(txtActiveRate.Text) / (UD.SM.FlowRate * UD.SM.NonVGrav), UN_RATEMASS)
  XPOND = UnitsInternal(Val(txtCalc(0).Text), UN_LENGTH)
  ...
  Call agtox(UD, _
             ...
             AQTYPE, INTYPE, LPOND, DPOND, XACT, XPOND, _
  ...
  Else
    txtCalc(0).Text = AGFormat$(UnitsDisplay(XPOND, UN_LENGTH))
  End If
  If XAPPL < 0 Then
...
Private Sub cmdExport_Click()
  ...
  If Not NeedCalcs Then
    If GenPlotTitles(PV_AQUA, False) And GenPlotUnits(PV_AQUA) Then
...
Private Sub cmdPlot_Click()
  ...
  GenPlotTitleStrings PV_AQUA, False, strTitle, s, s, lngID
  PlotSetDataTitle 0, strTitle + ", width=" + txtDist.Text + " " + lblDistUnits.Caption
    
  'plot
  If SetupPlot(PV_AQUA) Then frmPlot.Show vbModal
...
'This routine is executed each time the form is shown
  ...
  If UP.Units <> PreviousUnits Then
    ...
    For Each c In optBodyType
      ...
      End If
    Next
    'Convert any existing user-defined values if this is not the first time the form is shown
    If PreviousUnits <> -1 Then
      ...
      If txtDepth.Text <> "" Then
        txtDepth.Text = AGFormat$(UnitsDisplay(UnitsInternalSys(Val(txtDepth.Text), UN_LENGTH, PreviousUnits), UN_LENGTH))
      End If
    End If
...
'Generate report text for this form to be used for printing
  ...
  AppendStr gfd, "AgDRIFT® Aquatic Assessment", True
  AppendStr gfd, "", True
  
  AppendStr gfd, "Aquatic Body: " & optBodyType(BodyType).Caption, True
  AppendStr gfd, "  " & lblDist.Caption & " " & txtDist.Text & " " & lblDistUnits.Caption, True
  AppendStr gfd, "  " & lblDepth.Caption & " " & txtDepth.Text & " " & lblDepthUnits.Caption, True
  AppendStr gfd, "", True
  ...
  Else
    AppendStr gfd, "", True
  End If
  AppendStr gfd, "  " & lblResults3.Caption & " " & txtCalc(4).Text & " " & lblUnits4.Caption, False
  If CalcOutputMarker = 3 Then
    AppendStr gfd, " (input)", True
  Else
    AppendStr gfd, "", True
