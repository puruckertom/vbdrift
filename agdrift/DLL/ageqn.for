...
C
      ...
      COMMON /DVFV/ IYDV(60,51),IYFV(60,51)
C
...
10    NSTEP=NSTEP+1
      ...
      IF (ISTT.EQ.0) THEN
        ...
        ENDIF
      ENDIF
      TEM=2.0**AMAX1(2.0,4.0*AMIN1(1.0,(50.0/DMIN)**3))
      DT=DT*AMIN1(TEM,DMIN)
...
C
     ...
     $      CALL AGCAN(XOV(1,I),XNV(1,I),EDOV(I),EDNV(I),CNEW)
          ...
          IF (CTEM.GT.0.0) THEN
            ...
            DO N=2,NH
              HTEM=XOV(3,I)+(N-1)*DH
              IF (HTEM.GT.ZREF.AND.HTEM.LT.HCAN) NC=NC+1
            ENDDO
            IF (NC.GT.0) THEN
              TEMNC=1.0/NC
            ELSE
          ...
          DO N=2,NH
            ...
            IF (CTEM.GT.0.0) THEN
              IF (HTEM.GT.ZREF.AND.HTEM.LT.HCAN) THEN
                TAHFV(2,IH)=TAHFV(2,IH)+TEMNC*CTEM
                TAHFV(3,IH)=TAHFV(3,IH)+TEMNC*ATEM
              ENDIF
            ENDIF
          ENDDO
C
...
30        IF (T.GT.TATTV(NT)) THEN
     ...
     $                                  +YMASS*CNEW*(EDNV(I)/DIAM)**3
          ENDDO
          EFRAC=EFRAC+ETEM
          CALL AGDSD(YMASS*(CMASS(I)-CNEW),EDNV(I),DSCP)
          CMASS(I)=CNEW
C
          ...
          DO J=1,NSWTM
            ...
            IF (XNV(2,I).GT.TEM.AND.IYFV(I,J).EQ.0) THEN
              ...
              IYFV(I,J)=1
              CALL AGDSD(YTEM*YMASS*CMASS(I),EDNV(I),DSVP)
            ENDIF
C
            IF (ISW(I).LT.0) THEN
              TEF=(J-SFAC)*SWATH
              IF (XNV(2,I).LT.TEF) THEN
                CALL AGDSD(YTEM*YMASS*CMASS(I),EDNV(I),DSSB)
              ELSEIF (XNV(2,I).LT.TEM) THEN
                CALL AGDSD(YTEM*YMASS*CMASS(I),EDNV(I),DSDW)
              ENDIF
            ENDIF
          ENDDO
        ENDIF
      ...
      IF (XO.GT.0.0) THEN
        DO N=1,NVOR
          IF (ABS(GDKV(N)).GT.1.0E-10) THEN
            ...
            GDKV(N)=GDKV(N)*EXP(-AMIN1(ABS(GDK*BSTAB*DT/TEM),25.0))
          ENDIF
        ENDDO
      ENDIF
C
