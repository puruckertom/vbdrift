...
C  AGDISP Version 8.15 05/12/05
C
...
C
      ...
      IF (LFL.EQ.0) THEN
        ...
        CALL AGCHK(S,SMIN,SMAX,3,0.9*SMIN,1.1*SMAX,IER,FAC,REALWD)
        IF (IER.GT.0) THEN
          IF (IUNIT.EQ.0) THEN
            CHSTR='Air Speed (mph)'
          ELSE
...
C
      ...
      IF (LFL.EQ.1) THEN
        LFL=2
        FAC=1.0
        CALL AGCHK(A,DMIN,DMAX,3,0.9*DMIN,1.1*DMAX,IER,FAC,REALWD)
        IF (IER.GT.0) THEN
...
C
      ...
      IF (LFL.EQ.2) THEN
        ...
        FAC=1.0
        IF (R.EQ.0.0) THEN
          RR=5695.8991+178.3315*S-122.9546*A-1.00641*S*A
          CALL AGCHK(RR,RMIN,RMAX,3,0.9*RMIN,1.1*RMAX,IER,FAC,REALWD)
          IF (IER.GT.0) THEN
            IER=4
            CHSTR='Computed Rotation Rate (rpm)'
            JCHSTR=28
            RETURN
          ENDIF
        ELSE
          CALL AGCHK(R,RMIN,RMAX,3,0.9*RMIN,1.1*RMAX,IER,FAC,REALWD)
          IF (IER.GT.0) THEN
...
C
      ...
      IF (LFL.EQ.3) THEN
        ...
        CALL AGCHK(Q,QMIN,QMAX,3,0.09*QMIN,11.0*QMAX,IER,FAC,REALWD)
        IF (IER.GT.0) THEN
          IF (IUNIT.EQ.0) THEN
            CHSTR='Flow Rate (gpm)'
          ELSE
...
C
        LTEM=HHK.SPRTYPE
        CALL AGPARX(LTEM,ICLS,0,D50,RSNEW,NPTS,DDV,XXV)
      ENDIF
      RETURN
      END