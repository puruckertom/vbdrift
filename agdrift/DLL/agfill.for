...
C
      CHARACTER*40 CHSTR
C
      DIMENSION DDV(2),XXV(2),DKV(2),XKV(2),CKV(32),TNV(32)
...
C
        ...
        IF (IER.EQ.0) THEN
          ...
          ELSEIF (ITYPE.EQ.1) THEN
            I=1
            CALL AGKRR(NOLD,DKV,CKV,DNV(1,1),XNV,PSAVE,IER)
            IF (PSAVE.LT.0.99999) THEN
              I=2
              CALL AGKRR(NOLD,DKV,CKV,DNV(1,2),XNV,PSAVE,IER)
              IF (PSAVE.LT.0.99999) THEN
                I=3
                CALL AGKRR(NOLD,DKV,CKV,DNV(1,3),XNV,PSAVE,IER)
              ENDIF
            ENDIF
          ELSE
            I=1
            CALL AGKLN(NOLD,DKV,CKV,VMD,DNV(1,1),XNV,PSAVE,IER,L)
            IF (PSAVE.LT.0.99999) THEN
              I=2
              CALL AGKLN(NOLD,DKV,CKV,VMD,DNV(1,2),XNV,PSAVE,IER,L)
              IF (PSAVE.LT.0.99999) THEN
                I=3
                CALL AGKLN(NOLD,DKV,CKV,VMD,DNV(1,3),XNV,PSAVE,IER,L)
              ENDIF
            ENDIF
          ENDIF
        ENDIF
...
C
      ELSE
        IF (ITYPE.EQ.2) THEN
          IF (L.EQ.1) THEN
            IER=5
            CHSTR='Upper Limit Log Normal solution invoked'
            JCHSTR=39
          ELSEIF (L.EQ.2) THEN
            IER=5
            CHSTR='Bounded Log Normal solution invoked'
            JCHSTR=35
          ENDIF
        ENDIF
C
