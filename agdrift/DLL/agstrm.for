...
C
     ...
     $                  XWIDE,XDEEP,XACT,XDIST,XSRATE,XSLENG,
...
C                       1 = Write warning information plus real values
C                       2 = Write error information plus real values
C                       4 = Write error information string only
...
C
      ...
      IF (LFL.EQ.0) THEN
        ...
        CALL AGCHK(XWIDE,0.1,304.8,ITRTYP,0.1,DISTMX,IER,FAC,REALWD)
        IF (IER.NE.0) THEN
          IF (IUNIT.EQ.0) THEN
...
C
      IF (LFL.EQ.1) THEN
        ...
        FAC=1.0
        IF (IUNIT.EQ.0) FAC=3.2808
        CALL AGCHK(XDEEP,0.01,100.0,ITRTYP,0.01,100.0,IER,FAC,REALWD)
        IF (IER.NE.0) THEN
          IF (IUNIT.EQ.0) THEN
...
C
      IF (LFL.EQ.2) THEN
        LFL=3
        IF (UD.TIER.EQ.1.AND.XACT.LE.0.0) THEN
          IER=4
          IF (UD.SMOKEY.EQ.0) THEN
            CHSTR='Active Rate must be positive'
            JCHSTR=28
          ELSE
            CHSTR='Active Fraction must be positive'
            JCHSTR=32
          ENDIF
          RETURN
        ENDIF
...
C
      IF (LFL.EQ.3) THEN
        ...
        FAC=1.0
        IF (IUNIT.EQ.0) FAC=3.2808
        CALL AGCHK(XDIST,0.5*XWIDE,304.8-0.5*XWIDE,ITRTYP,
     $             0.5*XWIDE,DISTMX-0.5*XWIDE,IER,FAC,REALWD)
        IF (IER.NE.0) THEN
          IF (IUNIT.EQ.0) THEN
...
C
      IF (LFL.EQ.4) THEN
        ...
        CALL AGCHK(XSRATE,0.01,100000.0,ITRTYP,0.001,250000.0,
     $             IER,FAC,REALWD)
        IF (IER.NE.0) THEN
          IF (IUNIT.EQ.0) THEN
...
C
      IF (LFL.EQ.5) THEN
        ...
        FAC=1.0
        IF (IUNIT.EQ.0) FAC=3.2808
        CALL AGCHK(XSLENG,10.0,1000.0,ITRTYP,1.0,100000.0,
     $             IER,FAC,REALWD)
        IF (IER.NE.0) THEN
          IF (IUNIT.EQ.0) THEN
...
C
      IF (LFL.EQ.6) THEN
        LFL=7
        CALL AGCHK(XSTURN,0.0,60.0,ITRTYP,0.0,300.0,IER,1.0,REALWD)
        IF (IER.NE.0) THEN
...
C
      IF (LFL.EQ.7) THEN
        LFL=8
        CALL AGCHK(XRIPAR,0.0,0.5,ITRTYP,0.0,0.9,IER,1.0,REALWD)
        IF (IER.NE.0) THEN
...
C
      IF (LFL.EQ.8) THEN
        LFL=9
        CALL AGCHK(XDECAY,0.0,5.0,ITRTYP,0.0,25.0,IER,1.0,REALWD)
        IF (IER.NE.0) THEN
...
C
      IF (UD.TIER.EQ.1) THEN
        ...
        ELSEIF (UD.APPLMETH.EQ.1) THEN
          ...
          CALL AGGRNX(ITYPE,1,XDWND,NSBL,-1,NUMS,YN,ZN)
          SWATH=SSBLS
        ELSE
          NSBL=UD.OA.ENDTROW
          UO=2.235
          ...
          XDWND=UD.CTL.MAXDWND
          IBROW=UD.OA.BEGTROW
          CALL AGORCX(ITYPE,1,XDWND,IBTROW,NSBL,-1,NUMS,YN,ZN)
          SWATH=SSBLS
        ENDIF
        SDISP=0.0
...
10      IF (YN(NB+1).LE.XBEG) THEN
          NB=NB+1
          IF (NB.LT.NUMS) GO TO 10
        ENDIF
        NE=NB-1
20      IF (YN(NE+1).LE.XEND) THEN
          NE=NE+1
          IF (NE.LT.NUMS) GO TO 20
        ENDIF
        ...
        CV(NS)=100000.0*ACTIVE*XAPPL/XWIDE/XDEEP
      ENDDO
