...
'$Id: basUserLib.bas,v 1.1 2001/04/26 16:21:46 tom Exp $
'basUserLib.bas - User Library code
Option Explicit

'User Library Database name
'This string occupies the DBName field in the info table
'and identifies the database as an AgDRIFT User Library.
'This helps provent users from mixing up installed libraries
'and user libraries
Public Const USERLIBRARYDBNAME = "AgDRIFT User Library"

'User Library database format version
'Lib vers 1, AgDRIFT 2.00: Baseline user library format
...
'Create a new agdrift user library from scratch
...
'Open AgDRIFT User Library Database
...
TopUserLibOpen:
  ...
  'we are looking at a built-in AgDRIFT library, not a User Library.
...
ErrHandUserLibOpen:
  ...
  Case 3265, 3343, 10000
    MsgBox "Selected file is not an AgDRIFT User Library. Check Preferences", vbExclamation
    Resume ExitUserLibOpen
  Case 10001  '(user-defined) Library Version mismatch
    MsgBox "The AgDRIFT User Library file is the wrong version."
    Resume ExitUserLibOpen
  End Select
