...
' $Id: aircrlib.frm,v 1.6 2001/04/26 16:21:45 tom Exp $
...
'Set the Database table from which this form will draw
  ...
  'Type
  cboFilter(1).AddItem "Fixed-wing"
  cboFilter(1).AddItem "Helicopter"
...
'
  ...
  If cboFilter(0).ListIndex > 0 Then
    s = s + " Name= '" + cboFilter(0).List(cboFilter(0).ListIndex) + "'"
    needsep = True
  End If
  If cboFilter(1).ListIndex > 0 Then
    If needsep Then s = s + " AND"
    s = s + " Type="
    Select Case cboFilter(1).ListIndex
      Case 1 'fixed-wing
        s = s + "3"
      Case 2 'helicopter
        s = s + "4"
...
'update the caption of the position label
  lblPosition.Caption = Format$(PositionCount) + " of " + Format$(datAircraft.Recordset.RecordCount)
End Sub
