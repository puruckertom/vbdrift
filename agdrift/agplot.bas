...
'$Id: agplot.bas,v 1.15 2006/11/08 15:18:10 tom Exp $
'basAGPLOT - AgDRIFT plotting utilities
Option Explicit

'This is where all the work is done to define and produce
'the plots associated with AgDRIFT. Each plot is assigned
...
'
'if X1D is true, both np as X are defined to be
...
'PlotVar defines each individual AgDRIFT plot
...
Public Const PVC_T1AIRAF = PVA_T123 Or PVA_AIR Or PVA_AF
Public Const PVC_T2AIRAF = PVA_T23 Or PVA_AIR Or PVA_AF
Public Const PVC_T2AIRFS = PVA_T23 Or PVA_AIR Or PVA_FS
Public Const PVC_T3AIRAF = PVA_T3 Or PVA_AIR Or PVA_AF
Public Const PVC_T3AIRFS = PVA_T3 Or PVA_AIR Or PVA_FS
'
'PlotVars - combination of plot number and attributes
'
'(Tier I+)
Public Const PV_VFINC = 0 Or PVC_T3AIRAF Or PVA_UD Or PVA_MULT   'IVF all DSD's
Public Const PV_VFINC0 = 1 Or PVC_T1AIRAF Or PVA_UD Or PVA_LINE  'IVF for DSD 0 only
Public Const PV_VFINC1 = 2 Or PVC_T3AIRAF Or PVA_UD Or PVA_LINE  'IVF for DSD 1 only
Public Const PV_VFINC2 = 3 Or PVC_T3AIRAF Or PVA_UD Or PVA_LINE  'IVF for DSD 2 only
Public Const PV_VFCUM = 4 Or PVC_T3AIRAF Or PVA_UD Or PVA_MULT   'CVF all DSD's
Public Const PV_VFCUM0 = 5 Or PVC_T1AIRAF Or PVA_UD Or PVA_LINE  'CVF for DSD 0 only
Public Const PV_VFCUM1 = 6 Or PVC_T3AIRAF Or PVA_UD Or PVA_LINE  'CVF for DSD 1 only
Public Const PV_VFCUM2 = 7 Or PVC_T3AIRAF Or PVA_UD Or PVA_LINE  'CVF for DSD 2 only
'(Tier I+)
Public Const PV_DEP = 8 Or PVC_T1AGOAF Or PVA_UC Or PVA_LINE     'deposition
Public Const PV_PID = 9 Or PVC_T1AGOAF Or PVA_UC Or PVA_LINE     'pond-integrated deposition
'(Tier II+ Aerial)
Public Const PV_VERT = 10 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE     'vertical profile
Public Const PV_CONC = 11 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE     '1-hour average concentration
'(Tier III Aerial)
Public Const PV_COV = 12 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE      'cov/effective swath width
Public Const PV_FA = 13 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE       'Fraction Aloft
Public Const PV_MEAN = 14 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE     'Mean Deposition
Public Const PV_DWDSDINC = 15 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE 'Downwind DSD
Public Const PV_DWDSDCUM = 16 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE 'Downwind DSD
Public Const PV_FXDSDINC = 17 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE 'Vertical Flux DSD
Public Const PV_FXDSDCUM = 18 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE 'Vertical Flux DSD
'tbc reorder the plotvars
'(Tier II+ Aerial FS)
Public Const PV_TA = 20 Or PVC_T2AIRAF Or PVA_UC Or PVA_MULT       'Time Accountancy (all curves)
Public Const PV_TAALOFT = 21 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE  'Time Acc (aloft)
Public Const PV_TAVAPOR = 22 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE  'Time Acc (vapor)
Public Const PV_TACANOPY = 23 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE 'Time Acc (canopy)
Public Const PV_TAGROUND = 24 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE 'Time Acc (ground)
Public Const PV_HA = 25 Or PVC_T2AIRAF Or PVA_UC Or PVA_MULT       'Height Accountancy (all curves)
Public Const PV_HAALOFT = 26 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE  'Height Acc (aloft)
Public Const PV_HAVAPOR = 27 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE  'Height Acc (vapor)
Public Const PV_HACANOPY = 28 Or PVC_T2AIRAF Or PVA_UC Or PVA_LINE 'Height Acc (canopy)
Public Const PV_TAB = 29 Or PVC_T2AIRAF Or PVA_UC Or PVA_BAR       'Total Accountancy Bar Chart
Public Const PV_TAP = 30 Or PVC_T2AIRAF Or PVA_UC Or PVA_PIE       'Total Accountancy Pie Chart
'(Tier III Aerial FS)
Public Const PV_LAYOUT = 31 Or PVC_T3AIRFS Or PVA_UC Or PVA_LINE   'application layout
Public Const PV_DA = 32 Or PVC_T3AIRAF Or PVA_UC Or PVA_MULT       'Distance Acc
Public Const PV_DAALOFT = 33 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE  'Distance Acc (aloft)
Public Const PV_DAVAPOR = 34 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE  'Distance Acc (vapor)
Public Const PV_DACANOPY = 35 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE 'Distance Acc (canopy)
Public Const PV_DAGROUND = 36 Or PVC_T3AIRAF Or PVA_UC Or PVA_LINE 'Distance Acc (ground)
Public Const PV_SBDSDINC = 37 Or PVC_T3AIRFS Or PVA_UC Or PVA_LINE 'Spray Block DSD
Public Const PV_SBDSDCUM = 38 Or PVC_T3AIRFS Or PVA_UC Or PVA_LINE 'Spray Block DSD
Public Const PV_CNDSDINC = 39 Or PVC_T3AIRFS Or PVA_UC Or PVA_LINE 'Canopy DSD
Public Const PV_CNDSDCUM = 40 Or PVC_T3AIRFS Or PVA_UC Or PVA_LINE 'Canopy DSD
Public Const PV_SBDEP = 41 Or PVC_T3AIRFS Or PVA_UC Or PVA_LINE    'Spray Block Deposition
Public Const PV_SBCOVER = 42 Or PVC_T3AIRFS Or PVA_UC Or PVA_LINE  'Spray Block Area Coverage
Public Const PV_CANDEP = 43 Or PVC_T3AIRFS Or PVA_UC Or PVA_LINE   'Canopy Deposition
Public Const PV_SV = 19 Or PVC_T3AIRFS Or PVA_UD Or PVA_MULT       'Settling Velocity
Public Const PV_SVTANK = 56 Or PVC_T3AIRFS Or PVA_UD Or PVA_LINE   'Settling Velocity (Carrier)
Public Const PV_SVNONV = 57 Or PVC_T3AIRFS Or PVA_UD Or PVA_LINE   'Settling Velocity (Nonvol)
...
Public Const PV_TERR = 53 Or PVC_T1AGOAF Or PVA_TB Or PVA_LINE  'Terrestrial Assessment TB user-def pond dep
Public Const PV_SBLK = 54 Or PVC_T1AGOAF Or PVA_TB Or PVA_LINE  'Spray Block Assessment TB
Public Const PV_SBDET = 55 Or PVC_T3AIRFS Or PVA_TB Or PVA_LINE 'Spray Block Details
...
'is associated with the DSD.
  ...
  'Check Tier
  'current tier must be >= the minimum tier
  Select Case UD.Tier
  Case TIER_1
    If (PlotVar And PVA_T1) <> PVA_T1 Then Exit Function
  Case TIER_2
    If (PlotVar And PVA_T2) <> PVA_T2 Then Exit Function
  Case TIER_3
    If (PlotVar And PVA_T3) <> PVA_T3 Then Exit Function
  End Select
  ...
  Case AUD_SDTF
    If (PlotVar And PVA_AG) <> PVA_AG Then Exit Function
  Case AUD_FS
    If (PlotVar And PVA_FS) <> PVA_FS Then Exit Function
  End Select
  
  'Special case - Initial DSDs
  'In Tier 3 Individual DSD Plots are only allowed if the DSD is used
  'and composite DSD plots are only allowed if more than one DSD is used
  If UD.Tier = TIER_3 Then
    Select Case PlotVar
    Case PV_VFINC, PV_VFCUM
      If NumberOfDSDsUsed(UD) <= 1 Then Exit Function
    Case PV_VFINC0, PV_VFCUM0
      If Not DSDIsUsed(UD, 0) Then Exit Function
    Case PV_VFINC1, PV_VFCUM1
      If Not DSDIsUsed(UD, 1) Then Exit Function
    Case PV_VFINC2, PV_VFCUM2
      If Not DSDIsUsed(UD, 2) Then Exit Function
    End Select
  End If
...
'
  ...
  'and restore it after a non-line plot is produced.
  ...
  If UI.PlotVar <> 0 Then 'not the first plot produced
    If ((PlotVar And PVA_TYPE_MASK) = PVA_MULT) And _
       ((UI.PlotVar And PVA_TYPE_MASK) <> PVA_MULT) Then 'new=multi, prev=non-multi
      BackupPlotDataSources
    End If
    If ((PlotVar And PVA_TYPE_MASK) <> PVA_MULT) And _
       ((UI.PlotVar And PVA_TYPE_MASK) = PVA_MULT) Then 'new=non-multi, prev=multi
...
'                        words for neater column headings
  ...
  Select Case PlotVar
  Case PV_VFINC, PV_VFINC0, PV_VFINC1, PV_VFINC2, _
       PV_VFCUM, PV_VFCUM0, PV_VFCUM1, PV_VFCUM2 'incremental/cumulative volume fraction
    Select Case PlotVar
    Case PV_VFINC
    ...
    Case PV_VFINC0
      If UD.Tier < TIER_3 Then
        PlotTitle = "Incremental Volume Fraction"
      Else
        PlotTitle = "Incremental Volume Fraction DSD 1"
      End If
    Case PV_VFINC1
      PlotTitle = "Incremental Volume Fraction DSD 2"
    Case PV_VFINC2
      PlotTitle = "Incremental Volume Fraction DSD 3"
    Case PV_VFCUM
    ...
    Case PV_VFCUM0
      If UD.Tier < TIER_3 Then
        PlotTitle = "Cumulative Volume Fraction"
      Else
        PlotTitle = "Cumulative Volume Fraction DSD 1"
      End If
    Case PV_VFCUM1
      PlotTitle = "Cumulative Volume Fraction DSD 2"
    Case PV_VFCUM2
      PlotTitle = "Cumulative Volume Fraction DSD 3"
    End Select
    XTitle = "Drop Size"
  ...
  Case PV_FXDSDINC  'Vertical Flux DSD
    PlotTitle = "Vertical Profile Incremental DSD"
    XTitle = "Drop Size"
    YTitle = "Volume" + dlm + "Fraction"
    HelpID = 1110
  Case PV_FXDSDCUM  'Vertical Flux DSD
    PlotTitle = "Vertical Profile Cumulative DSD"
    XTitle = "Drop Size"
    YTitle = "Volume" + dlm + "Fraction"
    HelpID = 1110
  ...
  Case PV_SVNONV 'Settling Velocity
    PlotTitle = "Settling Velocity - Nonvolatile"
    XTitle = "Drop Size"
    YTitle = "Settling" + dlm + "Velocity" + dlm
    HelpID = 1467
  ...
  Case PV_CANDEP 'Canopy Deposition
    ...
    HelpID = 1470
  Case PV_TA, PV_TAALOFT, PV_TAVAPOR, PV_TACANOPY, PV_TAGROUND    'Time Accountancy
    PlotTitle = "Time Accountancy"
    XTitle = "Time"
    YTitle = "Fraction of" + dlm + "Tank Mix"
    HelpID = 1471
  Case PV_HA, PV_HAALOFT, PV_HAVAPOR, PV_HACANOPY    'Height Accountancy
    PlotTitle = "Height Accountancy"
    XTitle = "Fraction of Tank Mix"
    YTitle = "Vertical" + dlm + "Distance" + dlm
    HelpID = 1472
  Case PV_DA, PV_DAALOFT, PV_DAVAPOR, PV_DACANOPY    'Distance Accountancy
  ...
  Case PV_TAP    'Total Accountancy Pie Chart
    ...
    YTitle = ""
    HelpID = 1474
  Case PV_AQUA  'Aquatic Assessment Toolbox User-def pond-int depos
    PlotTitle = "Pond Integrated Deposition"
    XTitle = "Distance"
    YTitle = "Fraction of" + dlm + "Application" + dlm + "Rate"
    HelpID = 1511
...
'
  ...
  If GenPlotTitleStrings(PlotVar, FormatForPlots, _
    ...
    'Prepend "Tier xxx" and possibly "Ground"/"Orchard" to Title
    s = "Tier " + String$(UD.Tier, "I")
    If UD.ApplMethod <> AM_AERIAL Then s = s + " " & GetBasicNameAM(UD.ApplMethod)
    s = s + " " + PlotTitle
    PlotSetCaption s
    PlotSetPlotTitle s
...
'tbc    InitContourData 'finds min/max etc
  ...
  Case PV_VFINC, PV_VFCUM
    'These plots are only available in tier 3 when more than one DSD is used.
    Select Case PlotVar
    Case PV_VFINC
      PVtmp(0) = PV_VFINC0
      PVtmp(1) = PV_VFINC1
    ...
    slot = 0
    For iDSD = 0 To MAX_DSD - 1 'loop through all dsd's
      If DSDIsUsed(UD, iDSD) Then
        GenPlotDataUDUC UD, UC, PVtmp(iDSD), True, np, X(), Y(), Lbl()
        PlotXYDataResetSlot slot
        PlotDataAddPoints slot, np, X(), Y(), Lbl()
        PlotSetDataSource slot, "Current Data:"
        GenPlotDataTitleString UD, PVtmp(iDSD), DataTitle
        PlotSetDataTitle slot, DataTitle
        slot = slot + 1 'increment slot for next time
      End If
    Next
  Case PV_SV
    PVtmp(0) = PV_SVTANK
      ...
      If SourceIsValid(PlotVar, SourceID) Then
        ...
        Case SID_CURRENT 'Current Data
          If GenPlotDataUDUC(UD, UC, PlotVar, True, np, X(), Y(), Lbl()) Then
            PlotDataAddPoints slot, np, X(), Y(), Lbl()
            'Redo plot titles. Always redo them for Tier 1
            'because we don't know when settings have changed. For
            'other tiers, redo titles only when plotvar chenges. That way
            'any user modifications to the titles stay.
            If UD.Tier = TIER_1 Or UI.PlotVar <> PlotVar Then
        ...
        Case SID_T1LIB 'Tier I Library
          ApplMethod = Val(Param1)
          Select Case ApplMethod
          Case AM_AERIAL
            BasicType = Val(Param2)
            GenPlotDataTier1 PlotVar, AM_AERIAL, BasicType, 0, 0, np, X(), Y()
            PlotDataAddPoints slot, np, X(), Y(), Lbl()
        
          Case AM_GROUND
            BasicType = Val(Param2)
            NumSwaths = Val(Param3)
            GenPlotDataTier1 PlotVar, AM_GROUND, BasicType, NumSwaths, 0, np, X(), Y()
            PlotDataAddPoints slot, np, X(), Y(), Lbl()
        
          Case AM_ORCHARD
            BasicType = Val(Param2)
            StartSwath = Val(Param3)
            EndSwath = Val(Param4)
            GenPlotDataTier1 PlotVar, AM_ORCHARD, BasicType, StartSwath, EndSwath, np, X(), Y()
            PlotDataAddPoints slot, np, X(), Y(), Lbl()
          End Select
        
        Case SID_LIB 'Dropsize Library Entry
          Select Case PlotVar
          Case PV_VFCUM0, PV_VFCUM1, PV_VFCUM2 'Cumul Mass Frac
            ...
            Next
            PlotDataAddPoints slot, np, X(), Y(), Lbl()
          Case PV_VFINC0, PV_VFINC1, PV_VFINC2 'Incr Mass Frac
            'The library key is a multi-element comma-separated list
            'distributed throughout the ParamX's. We need to stitch
            'them all together.
            key = Param1
        ...
        Case SID_FTPRED 'Field Trial Prediction
          Select Case PlotVar
          Case PV_VFCUM0, PV_VFCUM1, PV_VFCUM2 'Cumul Mass Frac
            'The library key is a multi-element comma-separated list
            'distributed throughout the ParamX's. We need to stitch
          ...
          Case PV_VFINC0, PV_VFINC1, PV_VFINC2 'Incr Mass Frac
            ...
            GetTrialDataDSD key, np, X(), Y()
            PlotDataAddPoints slot, np, X(), Y(), Lbl()
          Case PV_DEP, PV_PID 'Depos, PID
            GetTrialDataPred PlotVar, Param1, np, X(), Y()
            For i = 0 To np - 1
              X(i) = UnitsDisplay(X(i), UN_LENGTH)
            Next
            PlotDataAddPoints slot, np, X(), Y(), Lbl()
          End Select
        
        Case SID_FTMEAS 'Field Trial Measurement
          Select Case PlotVar
          Case PV_DEP, PV_PID 'Depos, PID
            GetTrialDataMeas PlotVar, Param1, np, X(), Y()
            For i = 0 To np - 1
              X(i) = UnitsDisplay(X(i), UN_LENGTH)
            Next
            PlotDataAddPoints slot, np, X(), Y(), Lbl()
...
'tbc - does this need to be updated or removed?
  ...
  Case PV_VFCUM0
    np = xUD.DSD(0).NumDrop
    CMF = 0
    For i = 0 To xUD.DSD(0).NumDrop - 1
      CMF = CMF + xUD.DSD(0).MassFrac(i) 'generate cumulative mass fractions
      X(i) = xUD.DSD(0).Diam(i)
      Y(i) = CMF
      Lbl(i) = ""
    Next
  Case PV_VFCUM1
    np = xUD.DSD(1).NumDrop
    CMF = 0
    For i = 0 To xUD.DSD(1).NumDrop - 1
      CMF = CMF + xUD.DSD(1).MassFrac(i) 'generate cumulative mass fractions
      X(i) = xUD.DSD(1).Diam(i)
      Y(i) = CMF
      Lbl(i) = ""
    Next
  Case PV_VFCUM2
    np = xUD.DSD(2).NumDrop
    CMF = 0
    For i = 0 To xUD.DSD(2).NumDrop - 1
      CMF = CMF + xUD.DSD(2).MassFrac(i) 'generate cumulative mass fractions
      X(i) = xUD.DSD(2).Diam(i)
      Y(i) = CMF
      Lbl(i) = ""
    Next
  Case PV_VFINC0
  ...
  Case PV_VFINC2
    ...
    For i = 0 To xUD.DSD(2).NumDrop - 1
      X(i) = xUD.DSD(2).Diam(i)
      Y(i) = xUD.DSD(2).MassFrac(i)
      Lbl(i) = ""
    Next
  Case PV_SVTANK
    agsetl xUD.SM, npl, X(0), Y(0), Xtbc(0), Ytbc(0)
    np = npl
    For i = 0 To np - 1
    ...
    Next
  Case PV_SVNONV
    agsetl xUD.SM, npl, Xtbc(0), Ytbc(0), X(0), Y(0)
    np = npl
    For i = 0 To np - 1
      Y(i) = UnitsDisplay(Y(i), UN_SPEED)
      Lbl(i) = ""
...
Public Sub GenPlotDataTier1(PlotVar As Long, AppMeth As Integer, _
                            BasicType As Integer, _
                            Param1 As Integer, Param2 As Integer, _
                            np As Integer, X() As Single, Y() As Single)

'generate Tier1 plot data from ASAE data or similar
'PlotVar   i  Plot variable. one of the PV_* flags
'
'AppMeth   i  Tier 1 application method
'
'BasicType i
'
'Param1,2  i  additional information
'                       Param1   Param2
'               Aerial:   N/A      N/A
'               Ground: NumSwaths  N/A
'              Orchard: BegRow   EndRow
'np        o  number of data points returned
'X()       o  array of X values returned
'Y()       o  array of Y values returned
'
  Dim CMF As Single
  Dim NStmp As Long
  Dim NXtmp As Long
  Dim NumSwaths As Long
  Dim BegTrow As Long
  Dim EndTrow As Long
  Dim dep_flag As Long
  Dim Ldum As Long
  Dim xDSD As DropSizeDistData
  Dim i As Integer
    
  Dim Tier As Long
  Dim MaxDownwindDist As Single

  Tier = CLng(UD.Tier) 'Tier affects aglibr and aggrnd
  MaxDownwindDist = UD.CTL.MaxDownwindDist 'aglibr and aggrnd require this for Tier 3
    
  np = 0 'default return value
   
  Select Case PlotVar
  Case PV_VFCUM0, PV_VFCUM1, PV_VFCUM2  'cumulative mass fraction
    Select Case AppMeth
    Case AM_AERIAL
      'get the basic plot data
      GetBasicDataDSD BasicType, xDSD
      np = xDSD.NumDrop
      CMF = 0
      For i = 0 To np - 1
        DoEvents
        X(i) = xDSD.Diam(i)
        CMF = CMF + xDSD.MassFrac(i) 'generate cumulative mass fractions
        Y(i) = CMF
      Next
    End Select
  
  Case PV_VFINC0, PV_VFINC1, PV_VFINC2  'incremental mass fraction
    Select Case AppMeth
    Case AM_AERIAL
      'get the basic plot data
      GetBasicDataDSD BasicType, xDSD
  ...
  Case PV_DEP, PV_PID  'deposition, pond-integrated deposition
    If PlotVar = PV_DEP Then dep_flag = AGGRND_DEPOS
    If PlotVar = PV_PID Then dep_flag = AGGRND_PID
    Select Case AppMeth
    Case AM_AERIAL
      'get the raw distribution from the lib
      GetBasicDataDEP BasicType, np, X(), Y()
      'clean up the distribution
      '(note that UD.CTL.MaxDownwindDist is only used in Tier 3)
      NXtmp = CLng(np)
      Call aglibr(Tier, dep_flag, MaxDownwindDist, NXtmp, X(0), Y(0))
      np = CInt(NXtmp)
    Case AM_GROUND
      NumSwaths = CLng(Param1)
      Call aggrnd(CLng(BasicType), Tier, MaxDownwindDist, NumSwaths, dep_flag, NXtmp, X(0), Y(0))
      np = CInt(NXtmp)
      Param1 = CInt(NumSwaths) 'NunSwaths may have been corrected
    Case AM_ORCHARD
      BegTrow = CLng(Param1)
      EndTrow = CLng(Param2)
      Call agorch(CLng(BasicType), Tier, MaxDownwindDist, BegTrow, EndTrow, dep_flag, NXtmp, X(0), Y(0))
      np = CInt(NXtmp)
    End Select
    'change to display units
    For i = 0 To np - 1
      X(i) = UnitsDisplay(X(i), UN_LENGTH)
    Next
  End Select
...
'
  ...
  Select Case PlotVar
  Case PV_VFCUM, PV_VFCUM0, PV_VFCUM1, PV_VFCUM2, _
       PV_VFINC, PV_VFINC0, PV_VFINC1, PV_VFINC2, _
       PV_DWDSDINC, PV_DWDSDCUM, _
       PV_FXDSDINC, PV_FXDSDCUM, _
       PV_SBDSDINC, PV_SBDSDCUM, _
       PV_CNDSDINC, PV_CNDSDCUM
    PlotSetYunits ""
    PlotSetXunits " (µm)"
  Case PV_SV, PV_SVTANK, PV_SVNONV
    PlotSetYunits " (" & UnitsName(UN_SPEED) & ")"
    PlotSetXunits " (µm)"
  Case PV_DEP, PV_PID, PV_COV, PV_FA, PV_MEAN, PV_SBDEP, _
       PV_MAA, PV_AQUA, PV_TERR, PV_DA, PV_LAYOUT
    PlotSetYunits ""
...
Public Function GenPlotDataTitleString(xUD As UserData, PlotVar As Long, DataTitle As String)
  ...
  Case PV_VFINC0, PV_VFCUM0
    ...
    Case TIER_1
      DataTitle = GetBasicNameT1(xUD)
    Case TIER_2
      DataTitle = "Initial DSD"
    Case TIER_3
      DataTitle = "Initial DSD 1"
    End Select
  Case PV_VFINC1, PV_VFCUM1
    DataTitle = "Initial DSD 2"
  Case PV_VFINC2, PV_VFCUM2
    DataTitle = "Initial DSD 3"
  Case PV_DEP 'deposition
    Select Case xUD.Tier
    Case TIER_1
      DataTitle = GetBasicNameT1(xUD)
  ...
  Case PV_PID 'pond-integrated deposition
    ...
    End Select
  Case PV_VERT 'Vertical Profile
    DataTitle = "Vertical Profile at " & _
  ...
  Case PV_FXDSDINC  'Vertical Flux DSD
    DataTitle = "Vertical Profile Incremental DSD at " & _
                AGFormat$(UnitsDisplay(xUD.CTL.FluxPlane, UN_LENGTH)) & _
                " " & UnitsName(UN_LENGTH)
  Case PV_FXDSDCUM  'Vertical Flux DSD
    DataTitle = "Vertical Profile Cumulative DSD at " & _
                AGFormat$(UnitsDisplay(xUD.CTL.FluxPlane, UN_LENGTH)) & _
                " " & UnitsName(UN_LENGTH)
  Case PV_SV
  ...
  If PlotIsAvailable(PlotVar) Then
    ...
    Else
      ...
      Case SID_NONE, SID_CURRENT, SID_FILE
        ...
        SourceIsValid = True
      Case SID_T1LIB    'Tier 1 Library
        'Tier 1 functions provide DSD's and depositions for all Tiers, SDTF
        Select Case PlotVar
        Case PV_VFCUM0, PV_VFCUM1, PV_VFCUM2, _
             PV_VFINC0, PV_VFINC1, PV_VFINC2, _
             PV_DEP, PV_PID
          If UD.Smokey = AUD_SDTF Then SourceIsValid = True
        End Select
      Case SID_LIB      'Drop Library
        'The Drop library provides DSD's for Tier II and III
        Select Case PlotVar
        Case PV_VFCUM0, PV_VFCUM1, PV_VFCUM2, _
             PV_VFINC0, PV_VFINC1, PV_VFINC2
          If UD.Tier >= TIER_2 Then SourceIsValid = True
        End Select
      Case SID_FTPRED   'Field Trial Prediction
        'The Field Trial Prediction database provides
        'Depositions (including Pond-Integrated) for Tier III SDTF
        Select Case PlotVar
        Case PV_DEP, PV_PID
          If UD.Tier = TIER_3 And _
             UD.Smokey = AUD_SDTF And _
             FieldTrialDataExists() Then SourceIsValid = True
        End Select
      Case SID_FTMEAS   'Field Trial Measurement
        'The Field Trial Measurement database provides
        'Depositions for Tier III SDTF
        Select Case PlotVar
        Case PV_DEP
          If UD.Tier = TIER_3 And _
             UD.Smokey = AUD_SDTF And _
