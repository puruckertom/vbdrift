...
' $Id: aircraft.frm,v 1.12 2006/11/08 15:18:10 tom Exp $
...
'transfer local data to form controls for editing
  ...
  cboBasicType.ListIndex = AC.BasicType     'combo box for Basic Type
  PropTakeAction = temp
...
'get new data
  ...
  Select Case NewType
    ...
    Case 2, 3 'library
      ...
      Else
        frmAircraftLib.SelectTable "AircraftFS"
      End If
...
'change the Properties controls to reflect the new Basic setting
  ...
  If PropTakeAction Then
    ...
    AC.Name = GetBasicNameAC(CInt(cboBasicType.ListIndex))
    GetBasicDataAC CInt(cboBasicType.ListIndex), AC
  
    'Place the new data in the controls
    DataToForm
...
'calculate and display Nozzle Distribution Extent
  ...
  If optType(0).Value Then 'basic
    'Ask about loading the basic nozzles
    s = "You are about to change to an aircraft which " + _
        "carries a pre-defined nozzle distribution. This " + _
        "nozzle distribution will replace the current " + _
        "nozzle distribution."
    Select Case MsgBox(s, vbOKCancel + vbQuestion)
    Case vbOK
      GetBasicDataNZ AC.BasicType, UD.NZ 'load basic nozzles
    Case vbCancel
      Exit Sub
    End Select
  Else 'non-basic aircraft type
    'Ask about adjusting nozzles
    sExtent = AGFormat$(65)
    sNozzles = ""
    sSpacing = AGFormat$(UnitsDisplay(0.3048, UN_LENGTH)) '1.0 ft
    
    s = "You are about to change aircraft. " + _
        "AgDRIFT can automatically adjust the nozzle " + _
    ...
    s = s + " and a spacing of " + sSpacing + " " + UnitsName(UN_LENGTH) + _
            ...
            Chr$(34) + "Nozzles and DSD" + Chr$(34) + ".) " + _
            "Make this change?"
...
Private Sub Form_Load()
  ...
  'init the Aircraft Type combo box
  cboBasicType.Clear
  For i = 0 To 3  'tbc replace static number with dynamic sizing somehow
    cboBasicType.AddItem GetBasicNameAC(i)
  Next
