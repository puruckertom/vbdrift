VERSION 5.00
Begin VB.Form frmTier2air 
   BorderStyle     =   0  'None
   Caption         =   "Tier II Aerial Agricultural Input"
   ClientHeight    =   6795
   ClientLeft      =   1770
   ClientTop       =   3375
   ClientWidth     =   9480
   ...
   End
End
Attribute VB_Name = "frmTier2air"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
' $Id: tier2air.frm,v 1.9 2011/12/27 17:47:01 tom Exp $
...
Private Sub txtNvAmt_Change()
  If PropTakeAction Then
    If (UD.SM.FlowRate * UD.SM.NonVGrav) > 0 Then
      UD.SM.NVFrac = UnitsInternal(Val(txtNvAmt.Text), UN_RATEMASS) / _
                     (UD.SM.FlowRate * UD.SM.NonVGrav)
      UD.SM.ACFrac = UD.SM.NVFrac
...
'transfer stored data to form controls for Spray Material
  ...
  txtNvAmt.Text = AGFormat$(UnitsDisplay(UD.SM.NVFrac * UD.SM.FlowRate * UD.SM.NonVGrav, UN_RATEMASS))
...
Private Sub UpdateSprayMaterialFractions()
'Adjust NVfrac and ACfrac after a change to FlowRate or NonVGrav
'The idea is that the fractions beneath the displayed rates must
'change so that the displayed rates can remain the same.
  Dim NVamt As Single
  Dim ACamt As Single
  
  NVamt = UnitsInternal(Val(txtNvAmt.Text), UN_RATEMASS)
  ACamt = NVamt 'UnitsInternal(Val(txtActiveAmt.Text), UN_RATEMASS)
  If (UD.SM.FlowRate * UD.SM.NonVGrav) > 0 Then
    UD.SM.NVFrac = NVamt / (UD.SM.FlowRate * UD.SM.NonVGrav)
    UD.SM.ACFrac = UD.SM.NVFrac 'ACamt / (UD.SM.FlowRate * UD.SM.NonVGrav)
  Else
    UD.SM.NVFrac = 0
    UD.SM.ACFrac = 0
  End If
